...
import React, { useState, useRef } from 'react';
import { motion } from 'framer-motion';
import { Mic, Upload, Pause, FileText, Loader2, User, FileAudio, Download } from 'lucide-react';
import toast from 'react-hot-toast';
...
const AudioManagement: React.FC<AudioManagementProps> = () => {
  ...
  const fetchAudioFiles = async () => {
    ...
    try {
      const response = await fetch(`http://20.244.9.179/audio/files/${patientId}`);
  ...
  const fetchTranscription = async (audioId: number) => {
    ...
    try {
      const response = await fetch(`http://20.244.9.179/audio/transcribe/${audioId}`);
      ...
      const data = await response.json();
      setTranscriptionStatus('completed');
      setTranscriptionText(data.transcription || 'Transcription fetched successfully.');
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://20.244.9.179/audio/upload?patient_id=${encodeURIComponent(patientId)}`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (patientData && patientData.audio_files.length > 0) {
        const latestAudioFile = patientData.audio_files[patientData.audio_files.length - 1];
            ...
            <div className="mt-4">
              ...
              {patientData.audio_files.length > 0 ? (
                <div className="mt-2 space-y-2">
                  <h4 className="text-sm font-medium text-gray-700">Audio Files:</h4>
                  <div className="max-h-40 overflow-y-auto">
                    {patientData.audio_files.map((file) => (
                      <div key={file.id} className="p-3 bg-gray-50 rounded-md mb-2 flex justify-between items-center">
                        <div>
                ...
                <p className="text-sm text-gray-600">No audio files found for this patient.</p>
              )}
            </div>
          )}
        </motion.div>
        ...
        >
          ...
          <div className="space-y-4">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={handleTranscribe}
              disabled={!audioFile || isUploading || !patientId.trim() || isFetchingTranscription}
              className="w-full flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:bg-gray-400"
            >
              ...
              {isUploading ? 'Uploading...' : isFetchingTranscription ? 'Fetching Transcription...' : 'Upload & Transcribe Audio'}
            </motion.button>
